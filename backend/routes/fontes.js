...
// Rotas desativadas para Reddit e TikTok
router.post('/reddit', async (req, res) => {
  return res.status(503).json({
    sucesso: false,
    mensagem: 'Funcionalidade do Reddit temporariamente desativada',
    status: 'maintenance'
  });
});
...
router.post('/tiktok', async (req, res) => {
  return res.status(503).json({
    sucesso: false,
    mensagem: 'Funcionalidade do TikTok temporariamente desativada',
    status: 'maintenance'
  });
});
